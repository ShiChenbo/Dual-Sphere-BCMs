...
for i=1:length(freq)
    ...
    T1=Functions.Tsph_Layer(deg,f,[0.8*a a],[inf 15]);
    T2=Functions.Tsph_PEC(deg,f,a*0.5);    
    ...
    Val=diag(Val);
    save(['Case3_Data\' num2str(f*1e9),'.000000result.mat'],'F','Val');
end
%% Postprocess
Functions.Tracking('Case3_Data',fbeg,fend,fstp,a);