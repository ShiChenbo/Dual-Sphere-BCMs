...
for i=1:length(freq)
    ...
    T1=Functions.Tsph_DIE(deg,f,a,2);
    T2=Functions.Tsph_DIE(deg,f,a*0.75,8);
    ...
    S=I+2*T;
    S0=I+2*R1*T1*R1.';% set sphere 1 as background
    
    [F,Val]=eigs((S*S0'-I)/2,40);% extract the first 40 eigenvectors
    Val=diag(Val);
    save(['Case2_Data\' num2str(f*1e9),'.000000result.mat'],'F','Val');
end
%% Postprocess
Functions.Tracking('Case2_Data',fbeg,fend,fstp,a);